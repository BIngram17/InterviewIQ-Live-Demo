  ...
  handler: withApi(async (request) => {
    ...
    if (action === "review") {
      const changes = Array.isArray(raw?.changes) ? raw.changes.slice(0, 8).map((change) => ({
        ...
        kind: change?.kind === "needs-info" ? "needs-info" : "rewrite",
