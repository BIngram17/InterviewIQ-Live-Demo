...
import { countWords, coverLetterInRange, currentDateIso, endsWithOmission, mislabelsCompletedPastDate, requestsSkillDeletion, resumeContainsEvidence, safeChangeKind, safeChangeOperation } from "../lib/resume-review.js";
...
app.http("resumeTools", {
  ...
  handler: withApi(async (request) => {
    ...
    const raw = await completeJson({
      ...
      maxTokens: action === "cover-letter" ? 1400 : 4000,
    ...
    if (!coverLetterInRange(coverLetter, coverMinimum, coverMaximum)) {
      try {
        ...
        return {
          ...
          headline: text(raw?.headline, 180),
          coverLetter,
          notes: arrayOfText(raw?.notes, 5, 220),
          isDraft: true,
