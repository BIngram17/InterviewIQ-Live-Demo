...
const actions = new Set(["review", "suggestions", "cover-letter"]);
...
app.http("resumeTools", {
  ...
  handler: withApi(async (request) => {
    ...
    const instruction = action === "review"
      ? 'Assess fit and return JSON with shape {"headline":string,"score":number 1-100,"summary":string,"strengths":string[],"gaps":string[],"atsKeywords":string[],"nextSteps":string[]}.'
      : action === "suggestions"
        ? 'Recommend truthful, targeted edits without inventing experience. Return JSON with shape {"headline":string,"summary":string,"changes":[{"section":string,"currentIssue":string,"suggestion":string,"example":string}],"atsKeywords":string[]}.'
    ...
    const raw = await completeJson({
      ...
      maxTokens: action === "cover-letter" ? 1700 : 2200,
    ...
    if (action === "suggestions") {
      ...
      if (!changes.length) throw new ApiError(502, "The AI did not return complete resume suggestions.");
      return { action, headline: text(raw?.headline, 180), summary: text(raw?.summary, 1200), changes, atsKeywords: arrayOfText(raw?.atsKeywords, 14, 80) };
    }
