...
app.http("resumeTools", {
  ...
  handler: withApi(async (request) => {
    ...
    const normalizeReturnedChanges = (rawChanges, criterionIds, atsKeywords) => (Array.isArray(rawChanges) ? rawChanges : [])
      ...
      .filter((change) => change.kind !== "needs-info" || /\[[^\]]+\]/.test(change.example));
    ...
    if (action === "review-changes") {
      ...
      try {
        ...
        system:
          "You are InterviewIQ's expert resume editor. Generate up to four highest-impact, material, non-duplicate resume changes tied to the supplied scoring criteria. " +
          "Each change must target a specific unmatched or under-evidenced job requirement and use an exact supplied criterion id. Return an empty changes array when no material improvement is supported; never manufacture a gap or recommend an already-completed change. " +
